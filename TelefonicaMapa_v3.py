...
grouped = df.groupby(['sitio', 'tipo_plan', 'tecnologia', 'fecha']).agg({'sum_bytes':'sum'})
grouped = grouped.reset_index()
grouped = grouped.set_index('fecha')
df_master = grouped.groupby(['sitio', 'tipo_plan', 'tecnologia', pd.Grouper(freq='W-mon')]).sum()
        ...
        html.Div(
            ...
            children = [
                ...
                html.Div(
                    ...
                    children=[
                        html.Img(
                            className="logo", src=app.get_asset_url("telefonica-logo.png")
                        ),
                            ...
                            children=[
                                ...
                                html.Div(
                                    ...
                                    children=[
                                        dcc.Dropdown(
                                            id='tech_name',
                                            options=[{'label':str(b),'value':b} for b in sorted(df['tecnologia'].unique())],
                               ...
                               # Dropdown tipo plan
                                html.Div(
                                    ...
                                    children=[
                                        dcc.Dropdown(
                                            id='tipo_plan',
                                            options=[{'label':str(b),'value':b} for b in sorted(df['tipo_plan'].unique())],
...
def update_figure(datePicked,selectedLocation,chosen_tech,chosen_plan):
    df_sub = df[
                (df['fecha'].isin(datePicked)) &
                (df[''])
                (df['tecnologia'].isin(chosen_tech)) &
                (df['tipo_plan'].isin(chosen_plan))
                ]
